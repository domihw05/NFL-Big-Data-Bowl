...
## Some EDA with Play Clock at Snap
...
only trying to explore passing plays. 
```{r}
...
For the play clock at snap, the quantitative variables was split up into 3 
...
# Will add a summary of these tomorrow and still need to edit everything beneath this line.
...
heatmap_data <- plays_filtered |>
  mutate(yardsToGoBucket = cut(yardsToGo, breaks = c(0, 5, 10, 15, 20, 30), 
                               right = FALSE)) |>
  group_by(playClockCategory, yardsToGoBucket) |>
  summarize(success_rate = 
              mean(passResult == "C", na.rm = TRUE), .groups = "drop")

ggplot(heatmap_data, aes(x = playClockCategory, y = yardsToGoBucket, 
                         fill = success_rate)) +
  geom_tile() +
  labs(
