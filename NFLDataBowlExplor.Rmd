...
```
Here using the carat library we are able to provide summary statistics of the model. 
We can see the confusion matrix of the predicted.
```{r}
predicted_classes <- predict(multinom_model, type = "class")

plays_filtered$predicted_class <- predicted_classes

library(caret)
conf_matrix <- confusionMatrix(plays_filtered$predicted_class, plays_filtered$passResult)
print(conf_matrix)
```
```{r}
