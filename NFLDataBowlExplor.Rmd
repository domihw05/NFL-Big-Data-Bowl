...
Dom's Recent Exploration is below.
...
```{r}
dom_mov_data <- tracking |>
  filter(frameType == "AFTER_SNAP") |>
  group_by(gameId, playId, nflId)

tracking_roles <- dom_mov_data |>
...
tracking_pass_rush <- tracking_roles |>
  filter(position %in% c("SS","FS","DE","CB","ILB","OLB","DT")) |>
...
# was going to proceed to come up with a boolean table value that
# identifies whether a player is rushing the quarterback or not
# don't know how to get arround this error:
