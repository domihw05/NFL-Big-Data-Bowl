...
Down (Quantitative):
```{r}
ggplot(plays_filtered, aes(x = as.factor(down))) +
  geom_bar(fill = "steelblue", color = "black") +
  labs(title = "Distribution of Downs", x = "Down", y = "Count") +
  theme_minimal()
```
Pass Length (Quantitative):
```{r}
ggplot(plays_filtered, aes(x = passLength)) +
  geom_histogram(binwidth = 2, fill = "steelblue", color = "black") +
  labs(title = "Distribution of Pass Length", x = "Pass Length", y = "Count") +
  theme_minimal()
```
pff_passCoverage (Categorical):
```{r}
ggplot(plays_filtered, aes(x = pff_passCoverage)) +
  geom_bar(fill = "steelblue", color = "black") +
  labs(title = "Distribution of Pass Coverage", x = "Pass Coverage", y = "Count") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
dropbackType (Categorical):
```{r}
ggplot(plays_filtered, aes(x = dropbackType)) +
  geom_bar(fill = "steelblue", color = "black") +
  labs(title = "Distribution of Dropback Type", x = "Dropback Type", y = "Count") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
perfect for this kind of model. The model predicts the pass result using 
variables from the EDA above that were correlated with pass result: 
playClockAtSnap, down, yardsToGo, passLength, pff_passCoverage, dropbackType. To 
train the model, we have partitioned the data to a train_data and a test_data in 
which we will train the model on the train_data and test the model's success on 
the test_data. A common ratio for training and testing is 80 to 20 percent of 
the whole model which is used below. The code block below splits the 
plays_filtered data and trains the model named passResult_model. 
